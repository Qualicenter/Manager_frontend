...
const Desempenio = ({ etiqueta, porcentaje, color }) => {
  ...
  if (
    ...
    agente[0]?.data?.MetricResults?.length === 0
  ) {
    return (
  ...
  const nivelServicio =
    agente[0]?.data?.MetricResults[0]?.Collections?.find(
      (collection) => collection.Metric.Name === "SERVICE_LEVEL"
    )?.Value?.toFixed(2) || "";
  const tasaRespuesta =
    agente[0]?.data?.MetricResults[0]?.Collections?.find(
      (collection) => collection.Metric.Name === "AGENT_ANSWER_RATE"
    )?.Value?.toFixed(2) || "";
      ...
      <div className="contenedor-barra-de-progreso">
        ...
        >
          ...
          {porcentaje >= 0 && porcentaje <= 15 ? (
            <span style={{ transform: "translateX(120%)" }}>{porcentaje}%</span>
          ) : (
            <span className="porcentaje-transicion" style={{ zIndex: 1 }}>
              {porcentaje}%
            </span>
          )}
        </div>
